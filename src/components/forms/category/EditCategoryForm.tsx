...
const EditCategoryForm = ({ item }: PropData) => {
  ...
  useEffect(() => {
    if (item) {
      ...
      const files = item.images.map((itemData: CategoryImageModel) => {
        return {
          uid: itemData.image.id,
          name: item.name,
          status: "done" as UploadFileStatus,
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      await AppAxios.put(`/api/v1/categories/${item.id}`, formData)
        .then(res => {
          const { data } = res;
