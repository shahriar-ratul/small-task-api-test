...
import type { BrandImageModel, BrandModel } from "@/interfaces/BrandModel";
...
const EditBrandForm = ({ item }: PropData) => {
  ...
  useEffect(() => {
    if (item) {
      ...
      const files = item.images.map((itemData: BrandImageModel) => {
        return {
          uid: itemData.id,
          name: item.name,
          status: "done" as UploadFileStatus,
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      await AppAxios.put(`/api/v1/brands/${item.id}`, formData)
        .then(res => {
          const { data } = res;
          ...
          MySwal.fire({
            ...
            router.replace("/admin/purchase/brand");
          });
        })
        .catch(err => {
          MySwal.fire({
