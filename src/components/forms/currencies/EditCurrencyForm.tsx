...
import type { CurrencyModel } from "@/interfaces/CurrencyModel";
...
const EditCurrencyForm = ({ item }: PropData) => {
  // ** States
  ...
  const onSubmit = async (data: FormData) => {
    const { name, display_name, rate } = data;
    ...
    try {
      await AppAxios.put(`/api/v1/currencies/${item.id}`, formData)
        .then(res => {
          const { data } = res;
          ...
          MySwal.fire({
            ...
            router.replace("/admin/purchase/currency");
          });
        })
        .catch(err => {
          MySwal.fire({
