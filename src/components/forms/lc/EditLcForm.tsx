...
import type { LCModel } from "@/interfaces/LCModel";
...
const EditLcForm = ({ item }: PropData) => {
  // ** States
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      await AppAxios.put(`/api/v1/lcs/${item.id}`, formData)
        .then(res => {
          const { data } = res;
