...
import type { UnitModel } from "@/interfaces/UnitModel";
...
const EditUnitForm = ({ item }: PropData) => {
  // ** States
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      await AppAxios.put(`/api/v1/units/${item.id}`, formData)
        .then(res => {
          const { data } = res;
          ...
          MySwal.fire({
            ...
            router.replace("/admin/purchase/unit");
          });
        })
        .catch(err => {
          MySwal.fire({
