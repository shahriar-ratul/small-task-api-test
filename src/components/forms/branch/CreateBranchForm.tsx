...
const CreateBranchForm = () => {
  ...
  const onSubmit = async (data: BranchFormData) => {
    ...
    const formData = {
      ...
      card_images: idFileList.map(item => item.uid),
      attachments: attachmentFileList.map(item => item.uid),
      certificates: businessFileList.map(item => item.uid)
    };

    try {
      await AppAxios.post("/api/v1/branches", formData)
        .then(res => {
          // console.log(res);
          const { data } = res;
          ...
          MySwal.fire({
            title: "Success",
            text: data.data.message || "Branch Added successfully",
            icon: "success"
          }).then(() => {
