...
import type { OccasionModel } from "@/interfaces/OccasionModel";
...
const EditOccasionForm = ({ item }: PropData) => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      await AppAxios.put(`/api/v1/occasions/${item.id}`, formData)
        .then(res => {
          const { data } = res;
          ...
          MySwal.fire({
            ...
            router.replace("/admin/purchase/occasion");
          });
        })
        .catch(err => {
          MySwal.fire({
