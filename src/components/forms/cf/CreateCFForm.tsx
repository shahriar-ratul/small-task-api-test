...
const CreateCFForm = () => {
    ...
    try {
      if (current === 0) {
        ...
        setFormValues({
          ...
          owner_dob: selectOwnerDob ? selectOwnerDob.format("YYYY-MM-DD") : ""
        });
      } else if (current === 2) {
        await form.validateFields([
          /*  "contact_person_name",
  ...
  const onSubmit = async () => {
    ...
    try {
      await AppAxios.post("/api/v1/cfs", formData)
        .then(res => {
          const { data } = res;
              ...
              <Form.Item
                ...
                rules={
                  [
                    /* {
                    required: true,
                    message: "Please input your alt phone!"
                  }, */
                    /* {
                    pattern: new RegExp(/^(01)[0-9]{9}$/),
                    message: "Please enter correct BD Phone number."
