...
const EditPurchaseForm = ({ item }: PropData) => {
  ...
  const [selectedCf, setSelectedCf] = useState<any>(null);
  ...
  const [supplierBillDate, setSupplierBillDate] = useState<any>(null);
  ...
  useEffect(() => {
    if (item) {
      if (item.supplier) {
        ...
        form.setFieldsValue({ supplier: item.supplier.id });
      }
      setSelectedType(item.type);
      if (item.cf) {
        setSelectedCf(item.cf.id);
        form.setFieldsValue({ cf: item.cf.id });
      }
      if (item.lc) {
        setSelectedLc(item.lc.id);
        form.setFieldsValue({ lc: item.lc.id });
      }

      if (item.productOrigin) {
        setSelectedProductOrigin(item.productOrigin.id);
        form.setFieldsValue({ productOrigin: item.productOrigin.id });
      }

      if (item.currency) {
        setSelectedCurrency(item.currency.id);
        form.setFieldsValue({ currency: item.currency.id });
      }

      setIsActive(item.base.is_active);

      setFileList(
        item.images.map((imageData: any) => {
          return {
            uid: imageData.image.id,
            name: item.type,
            status: "done" as UploadFileStatus,
            url: imageData.image.path
          };
        })
      );
      ...
      form.setFieldsValue({
        ...
        cf_rate: item.cf_rate,
        gst: item.gst,
        lc_cost: item.lc_cost,
        other_cost: item.other_cost
      });
      if (item.supplier_bill_date) {
        form.setFieldsValue({
          supplier_bill_date: dayjs(item.supplier_bill_date)
        });
        setSupplierBillDate(dayjs(item.supplier_bill_date));
      }
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      await AppAxios.put(`/api/v1/purchases/${item.id}`, formData)
        .then(res => {
          const { data } = res;
