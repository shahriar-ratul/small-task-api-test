...
import type { SupplierModel } from "@/interfaces/SupplierModel";
...
const EditSupplierForm = ({ item }: PropData) => {
  ...
  const [selectedPurchaseType, setSelectedPurchaseType] = useState<any>(null);

  const [selectedOwnerGender, setSelectedOwnerGender] = useState<any>(null);
  const [selectedContactGender, setSelectedContactGender] = useState<any>(null);
  ...
  const next = async () => {
    try {
      if (current === 0) {
        ...
        setFormValues({
          ...formValues,
          shop_name: form.getFieldValue("shop_name"),
  ...
  const handlePurchaseTypeChange = (value: any) => {
    form.setFieldsValue({ supplier_purchase_type: value });
  ...
  const handleSupplierTypeChange = (value: any) => {
    setSelectedSupplierType(value as any);
  };

  const getSupplierTypes = async () => {
    const res = await AppAxios.get("/api/v1/common/all-supplier-types");
    if (res.data.success) {
      if (res.data.data.items.length > 0) {
        const items = res.data.data.items.map((item: any) => {
          return {
  ...
  useEffect(() => {
    if (item) {
      ...
      setSelectedSupplierType(item.supplierType.id);
      ...
      setSelectedPurchaseType(item.supplier_purchase_type);
      setSelectOwnerDob(item.owner_dob ? dayjs(item.owner_dob) : null);
      setSelectContactDob(
        item.contact_person_dob ? dayjs(item.contact_person_dob) : null
      );
      setSelectedContactGender(item.contact_person_gender);
      setSelectedOwnerGender(item.owner_gender);
  ...
  const onSubmit = async () => {
    ...
    const formData = {
      ...
      address_2: address_2,
      zip_code: zip_code,
      supplier_purchase_type: selectedPurchaseType,

      supplier_type_id: selectedSupplierType,
    ...
    try {
      await AppAxios.put(`/api/v1/suppliers/${item.id}`, formData)
        .then(res => {
          const { data } = res;
          ...
          MySwal.fire({
            ...
            router.replace("/admin/csrm/supplier");
          });
        })
        .catch(err => {
          MySwal.fire({
    ...
    <>
                ...
                /* rules={[
      {
        ...
        message: "Please input your phone!"
              ...
              <Form.Item
                label="Supplier Purchase Type"
                style={{
                  marginBottom: 0
                }}
                name="supplier_purchase_type"
              >
