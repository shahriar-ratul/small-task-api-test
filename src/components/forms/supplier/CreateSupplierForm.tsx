...
const CreateSupplierForm = () => {
  ...
  const [selectedPurchaseType, setSelectedPurchaseType] = useState("local");
  ...
  const onOwnerDateChange: DatePickerProps["onChange"] = (date, dateString) => {
    ...
    /* else {
      setSelectOwnerDob(dayjs());
      form.setFieldsValue({ owner_dob: dayjs() });
    } */
  };
  ...
  const onContactDateChange: DatePickerProps["onChange"] = (
    ...
    /* else {
      setSelectContactDob(dayjs());
      form.setFieldsValue({ contact_person_dob: dayjs() });
    } */
  };
  ...
  const handleSupplierTypeChange = (value: any) => {
    form.setFieldsValue({ supplier_type: value });
  ...
  const onSubmit = async () => {
    ...
    try {
      await AppAxios.post("/api/v1/suppliers", formData)
        .then(res => {
          const { data } = res;

          MySwal.fire({
            title: "Success",
            text: data.data.message || "Supplier Added successfully",
            icon: "success"
          }).then(() => {
        ...
        >
            ...
            <>
              ...
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: "Please input your supplier type!"
                  }
                ]}
              >
          ...
          {current === 2 && (
                ...
                /*   rules={[
            {
              ...
              >
                <Input
                  type="contact_person_phone"
                  placeholder="contact_person_phone"
                  className={`form-control`}
                  name="contact_person_phone"
                />
              </Form.Item>
