...
import type {
  SubCategoryImageModel,
  SubCategoryModel
} from "@/interfaces/SubCategoryModel";
...
const EditSubCategoryForm = ({ item }: PropData) => {
  ...
  useEffect(() => {
    if (item) {
      ...
      const files = item.images.map((itemData: SubCategoryImageModel) => {
        return {
          uid: itemData.id,
          name: item.name,
          status: "done" as UploadFileStatus,
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      await AppAxios.put(`/api/v1/subcategories/${item.id}`, formData)
        .then(res => {
          const { data } = res;
