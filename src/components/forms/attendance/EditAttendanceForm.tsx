...
import type { AttendanceModel } from "@/interfaces/AttendanceModel";
...
const EditAttendanceForm = ({ item }: PropData) => {
  ...
  useEffect(() => {
    if (item) {
      ...
      setStartTime(dayjs(item.start_time, "HH:mm:ss"));
      setEndTime(dayjs(item.end_time, "HH:mm:ss"));

      setIsHalfDay(item.is_half_day);
  ...
  const onSubmit = async (data: FormData) => {
    ...
    // check end time is greater than start time cant be same
    ...
    try {
      await AppAxios.put(`/api/v1/attendances/${item.id}`, formData)
        .then(res => {
          const { data } = res;
