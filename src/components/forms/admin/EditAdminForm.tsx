...
const EditAdminForm = ({ item }: PropData) => {
  ...
  useEffect(() => {
    if (item) {
      ...
      setIsOwner(item.is_owner);
      setSelectedEmployee(item.employee ? item.employee.id : null);
      setSelectedOwner(item.owner ? item.owner.id : null);

      const files = item.images.map((image: any) => {
        return {
          uid: image ? image.image.id : "",
          name: item.username,
          status: "done" as UploadFileStatus,
  ...
  const onSubmit = async (data: AdminFormData) => {
    ...
    try {
      await AppAxios.put(`/api/v1/admins/${item.id}`, formData)
        .then(res => {
          const { data } = res;
